...
private:
    ...
    void resizeMatrix() {
        ...
        for (int i = 0; i < newCapacity; ++i) {
            if (i < capacity) newExists[i] = exists[i];
            else newExists[i] = false;
        }
    ...
    void check_cap(int v) {
        while (v >= capacity) resizeMatrix();
    }

public:
    AdjacencyMatrixGraph(int initialCap = 100) {
        capacity = initialCap;
        ...
        for (int i = 0; i < capacity; ++i) exists[i] = false;
    }
    ...
    void FindShortestPath(int u, int v) const override {
        ...
        if (!visited[v]) {
            ...
            int path[capacity];
            int len = 0;
            for (int at = v; at != -1; at = prev[at]) path[len++] = at;
    ...
    linkedList GetNeighbors(int u) const override {
        ...
        init(&neighbors);
        if (u >= capacity || !exists[u]) return neighbors;

        for (int i = 0; i < capacity; ++i) {
            if (matrix[u][i] && exists[i]) append(i, &neighbors);
        }
        return neighbors;
    }
};
