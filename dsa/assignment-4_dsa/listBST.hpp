...
private:
    ...
    bool insertHelp(Node* cur, Key k, Value v){
        if(k == cur->key) return false;
            ...
            else return insertHelp(cur->left, k,v);
        }
        else{
            ...
            else return insertHelp(cur->right, k,v);
        }
    }
   Node* removeHelp(Node* cur, Key k, bool &suc){
        if(cur == nullptr) return nullptr;
        else if (k< cur->key) cur->left = removeHelp(cur->left, k, suc);
        else if (k> cur->key) cur->right = removeHelp(cur->right,k, suc);
        else{
            suc = true;
            //1 . No child
    ...
    bool findHelp(Node* cur, Key k)const{
        ...
        else if (k <cur->key) return findHelp(cur->left, k);
        else return findHelp(cur->right, k);
    }

    Value getHelp(Node* cur, Key k)const{
        if(cur == nullptr) throw std::runtime_error("key not found");
        if(cur->key == k) return cur->value;
         else if (k <cur->key) return getHelp(cur->left, k);
        else return getHelp(cur->right, k);
    }
    void updateHelp(Node* cur, Key k, Value v){
         if(cur == nullptr) throw std::runtime_error("key not found");
        if(cur->key == k) cur->value = v;
         else if (k <cur->key)  updateHelp(cur->left, k,v);
        else  updateHelp(cur->right, k,v);
    }
    // size_t sizeHelp(Node* cur)const{
    //     if(cur==nullptr) return 0;
    //     size_t leftsize = sizeHelp(cur->left);
    //      size_t rightsize = sizeHelp(cur->right);
    //      return (leftsize+rightsize+1);
    // }
    Key minimumHelp(Node* cur)const{
        if(cur == nullptr) throw std::runtime_error("Tree is empty");
        else if(cur->left == nullptr) return cur->key;
        return minimumHelp(cur->left);
    }
    Key maximumHelp(Node* cur)const{
        if(cur == nullptr) throw std::runtime_error("Tree is empty");
        else if(cur->right == nullptr) return cur->key;
